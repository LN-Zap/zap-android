...
public class PendingClosingChannelViewHolder extends RecyclerView.ViewHolder {
    ...
    public PendingClosingChannelViewHolder(View v) {
        super(v);
    ...
    public void bindPendingClosingChannelItem(final PendingClosingChannelItem pendingClosingChannelItem) {


        // Set state
        mStatus.setText("pending closing");
        mStatusDot.setImageTintList(ColorStateList.valueOf(ContextCompat.getColor(mContext, R.color.gray)));
        mRootView.setAlpha(0.65f);


        // Set capacities
        long availableCapacity = pendingClosingChannelItem.getChannel().getChannel().getCapacity();
        float localBarValue = (float) ((double) pendingClosingChannelItem.getChannel().getChannel().getLocalBalance() / (double) availableCapacity);
        float remoteBarValue = (float) ((double) pendingClosingChannelItem.getChannel().getChannel().getRemoteBalance() / (double) availableCapacity);
        ...
        mLocalBalance.setText(MonetaryUtil.getInstance().getPrimaryDisplayAmountAndUnit(pendingClosingChannelItem.getChannel().getChannel().getLocalBalance()));
        mRemoteBalance.setText(MonetaryUtil.getInstance().getPrimaryDisplayAmountAndUnit(pendingClosingChannelItem.getChannel().getChannel().getRemoteBalance()));


        // Set name

        for (NodeInfo i : Wallet.getInstance().mNodeInfos) {
            if (i.getNode().getPubKey().equals(pendingClosingChannelItem.getChannel().getChannel().getRemoteNodePub())) {
                ...
                mRemoteName.setText(pendingClosingChannelItem.getChannel().getChannel().getRemoteNodePub());
            }
        }
