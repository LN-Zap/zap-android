...
public class WaitingCloseChannelViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bindWaitingCloseChannelItem(final WaitingCloseChannelItem waitingCloseChannelItem) {
        ...
        // Set capacities
        long availableCapacity = waitingCloseChannelItem.getChannel().getChannel().getCapacity();
        float localBarValue = (float) ((double) waitingCloseChannelItem.getChannel().getChannel().getLocalBalance() / (double) availableCapacity);
        float remoteBarValue = (float) ((double) waitingCloseChannelItem.getChannel().getChannel().getRemoteBalance() / (double) availableCapacity);
        ...
        mLocalBalance.setText(MonetaryUtil.getInstance().getPrimaryDisplayAmountAndUnit(waitingCloseChannelItem.getChannel().getChannel().getLocalBalance()));
        mRemoteBalance.setText(MonetaryUtil.getInstance().getPrimaryDisplayAmountAndUnit(waitingCloseChannelItem.getChannel().getChannel().getRemoteBalance()));


        // Set name

        for (NodeInfo i : Wallet.getInstance().mNodeInfos) {
            if (i.getNode().getPubKey().equals(waitingCloseChannelItem.getChannel().getChannel().getRemoteNodePub())) {
                ...
                mRemoteName.setText(waitingCloseChannelItem.getChannel().getChannel().getRemoteNodePub());
            }
        }
